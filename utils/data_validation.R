    ...
    if (var_name %in% colnames(data) &&
      ...
      if (length(group_by_vars) == 1 && group_by_vars == var_name) {
        ...
        duplicates <- data %>%
          ...
          summarise(unique_values = n_distinct(!!sym(var_name)),
                    .groups = "drop") %>%
        ...
        if (nrow(duplicates) > 0) {
          error_list[[var_name]] <- paste(
            ...
            "for:",
            paste(duplicates[[group_by_vars[1]]], collapse = ", ")
          )
        }
      }
    }
...
validate_data_file <- function(file, req_fields) {
  ...
  if (!check2) {
    error_list$check2 <-
      paste("Missing columns in 'Data':", paste(setdiff(required_columns, colnames(data)), collapse = ", "))
  }
  ...
  if (!check3) {
    error_list$check3 <-
      paste("Missing fields in 'Data Dictionary':",
            paste(setdiff(
              required_dict_fields, colnames(data_dict)
            ), collapse = ", "))
  }
  ...
  required_value_columns <-
    req_fields_data |> filter(missing_allowed == "FALSE") |> pull(var)
  ...
  if (check2 && check3) {
    ...
    missing_in_dict <-
      setdiff(additional_columns, dict_column_names)
    missing_in_data <-
      setdiff(dict_column_names, additional_columns)
    ...
    if (length(missing_in_dict) > 0 ||
      ...
      error_list$check8 <- paste(
        if (length(missing_in_dict) > 0)
          paste(
            "Columns in 'Data' not found in 'Data Dictionary':",
