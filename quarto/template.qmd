...
physical <- setNames(list(groups[[tr("physical_group")]]), tr("physical_group"))
...
texture <- dplyr::filter(producer_samples, abbr %in% c(tr("sand"), tr("silt"), tr("clay")))
...
```{r echo=FALSE}
qualities <- parse_bullets_from_markdown(section = "soil_qualities", content_dir = params$language)
...
indicators <- indicators[!apply(indicators, 1, function(row) all(is.na(row) | row == "")), ]
...
if (all(is.na(gis_ft[[tr("crop")]]))) {
  gis_ft <- gis_ft |> select(-all_of(tr("crop")))
}
# If longitude or latitude are empty, remove the columns from the printed table
if (all(is.na(gis_ft[[tr("longitude")]])) | all(is.na(gis_ft[[tr("latitude")]]))) {
  gis_ft <- gis_ft |> select(-any_of(c(tr("longitude"), tr("latitude"))))
}
