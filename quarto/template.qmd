...
dictionary <- readxl::read_xlsx(params$data_file, sheet = "Data Dictionary") |>
  dplyr::mutate(
    ...
    abbr_unit = gsub("<br>NA", "", abbr_unit),
...
if (all(is.na(gis_ft[[tr("crop")]]))) {
  gis_ft <- gis_ft |>
    dplyr::select(-dplyr::all_of(tr("crop")))
}
...
if (!NA %in% gis_df$longitude & !NA %in% gis_df$latitude) {
  ...
  # override get_zoom within get_tiles to avoid issue when total_tiles is less
  # than 4. this grabs the max zoom with with < 10 tiles
  res <- maptiles:::get_bbox_and_proj(df_sf)
  gz <- slippymath::bbox_tile_query(res$bbox_lonlat)
  zoom <- max(gz[gz$total_tiles < 10, "zoom"])
