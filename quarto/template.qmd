...
### READ IN DATA FILE
input <- readRDS(params$data_file)
...
```{r texture-group-tweaks}
# Determine which measurement group contains texture variables
texture_group <- soils::get_texture_group(dictionary)
texture_col <- tr("texture")

# Keep texture column only in the texture measurement group table
if (!is.null(texture_group) && texture_col %in% names(df_table)) {
  tables <- purrr::imap(
    tables,
    \(x, group_name) {
      if (group_name != texture_group &&
        texture_col %in% names(x)) {
        x <- dplyr::select(x, -dplyr::all_of(texture_col))
      }
      x
    }
  )
