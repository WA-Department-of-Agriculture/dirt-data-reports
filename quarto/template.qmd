...
params:
  ...
  project_results: "Intro to the project results section."
  project_summary: "This is a summary."
  looking_forward: "Next steps include more analysis."
  measures: ["def-texture.qmd", "def-bulk-density.qmd", "def-ph.qmd", "def-total-c.qmd", "def-total-n.qmd","def-min-c.qmd"]
---
...
measurement_dictionary <- read.csv(
  paste0(params$language, "/measurement_dictionary.csv"),
  encoding = "UTF-8"
)

# filtered dictionary
selected_measures <- measurement_dictionary |>
  dplyr::filter(file_name %in% measures)

# unique measure types, e.g. Biological, Physical, Etc
distinct_selected_measure_types <- selected_measures |>
  dplyr::distinct(section_name) |>
...
dictionary <- soils::sync_dictionary_texture(
  data,
  dictionary,
  language = stringr::str_to_title(params$language)
)
```
...
```{r tidy-long}
# OPTIONAL EDIT: If you have extra metadata columns in `data`, add them to this
# vector. All other columns are assumed to be quantitative measurements and will
# be coerced to numeric. Non-numeric values (e.g. "ND", "NA", "", "-", "<1")
# will be converted to NA and excluded from tables and plots.
metadata_cols <- c(
  "year",
  "sample_id",
  "farm_name",
...
# Coerce measurement columns to numeric. This will emit a warning of which
# columns have coerced a value to NA.
...
```{r get-producer-info}
# Ensure 'farm_name' exists; create if missing
if (!"farm_name" %in% names(results_long)) results_long$farm_name <- NA

# Fill missing farm names with fallback labels
...
# Helper to rename texture if it exists
safe_rename_texture <- function(df) {
  if ("Texture" %in% colnames(df)) {
    df <- dplyr::rename(df, !!tr("texture") := Texture)
  }
  if ("texture" %in% colnames(df)) {
    df <- dplyr::rename(df, !!tr("texture") := texture)
  }
...
crop_summary <- results_long |>
  ...
  soils::summarize_by_var(producer_samples, var = crop) |>
  dplyr::mutate(
    `Field or Average` = gsub("Fields", tr("fields"), `Field or Average`),
    `Field or Average` = gsub("Average", tr("average"), `Field or Average`)
  ) |>
  dplyr::rename(!!tr("field_or_average") := `Field or Average`) |>
  safe_rename_texture()

county_summary <- results_long |>
  dplyr::filter(!is.na(county)) |>
  soils::summarize_by_var(producer_samples, var = county) |>
  dplyr::mutate(
    `Field or Average` = gsub("Fields", tr("fields"), `Field or Average`),
    `Field or Average` = gsub("Average", tr("average"), `Field or Average`)
  ) |>
...
project_summary <- soils::summarize_by_project(results_long) |>
  dplyr::mutate(
    `Field or Average` = gsub("Fields", tr("fields"), `Field or Average`),
    `Field or Average` = gsub("Project Average", tr("project_average"), `Field or Average`)
  ) |>
  dplyr::rename(!!tr("field_or_average") := `Field or Average`) |>
  safe_rename_texture()
```

```{r combine-producer-and-summarize}
...
producer_table <- producer_samples |>
  dplyr::select(
    ...
    abbr,
    value,
    !!tr("field_or_average") := field_id,
    dplyr::any_of(
      if ("texture" %in% names(producer_samples)) "texture" else NULL
    )
  ) |>
  safe_rename_texture()
...
tables <- df_table |>
  ...
  purrr::map(\(x) {
    # Only include texture if present
    id_cols <- if (tr("texture") %in% names(x)) {
      c(tr("field_or_average"), tr("texture"))
    } else {
      tr("field_or_average")
    }
...
```{r physical-group-texture}
# Special wrangling for texture

# Only proceed if texture exists in any dataframe
if (any(purrr::map_lgl(tables, ~ tr("texture") %in% names(.x)))) {
  # Extract physical df from averages list
  if (tr("physical_group") %in% names(tables)) {
    physical <- setNames(
      list(tables[[tr("physical_group")]]),
      tr("physical_group")
    )
  } else {
    physical <- list()
  ...
  tables <- purrr::map(
    subset(
      tables,
      !(names(tables) == tr("physical_group"))
    ),
...
tables <- tables |>
  purrr::map(~ dplyr::filter(.x, !grepl(search, !!sym(tr("field_or_average")))))
...
headers <- results_long |>
  ...
  rlang::set_names() |>
  purrr::map(\(group) {
    soils::get_table_headers(dictionary, group) |>
...
<!-- WHAT WE MEASURED IN YOUR SOIL TEXT -->
...
for (measure_type in distinct_selected_measure_types) {
  ...
  measures_in_type <- selected_measures |>
    dplyr::filter(section_name == measure_type)

  # Measure group image path
  image <- paste0("![](images/", tolower(measures_in_type$type[1]), ".png)")

  # Header color
  color <- section_colors$color[section_colors$name == measures_in_type$type[1]]
...
for (measure_type in distinct_selected_measure_types) {
  ...
  image <- paste0(
    "![](images/",
    tolower(measures_in_type$type[1]),
    ".png){height='50px'}"
  )

  # Print the measure group header with image
  cat("##", measure_type, image, "\n\n")
...
flextable::as_grouped_data(indicators, groups = indicator_groups) |>
  ...
  flextable::bg(
    i = ~ !is.na(eval(sym(tr("measure_every")))),
    bg = lighter_color
  ) |>
  flextable::set_table_properties(layout = "autofit")
```
...
<!-- SOIL TESTING TEXT -->
`r get_text(section = "testing", content_dir = params$language)`
...
if (soils::is_column_empty(gis_ft[[tr("crop")]])) {
  gis_ft <- gis_ft |> dplyr::select(-c(tr("crop")))
}

# If longitude or latitude are empty, remove the columns from the printed table
if (soils::is_column_empty(gis_ft[[tr("longitude")]]) ||
  soils::is_column_empty(gis_ft[[tr("latitude")]])) {
  gis_ft <- gis_ft |>
    dplyr::select(-(c(tr("longitude"), tr("latitude"))))
}
...
`r get_text(section = "project_results", content_dir = params$language)` `r params$project_results`
...
list(
  ...
  downloadthis::download_this(
    ...
    button_label = tr("download_button"),
    button_type = "success"
  )
```
