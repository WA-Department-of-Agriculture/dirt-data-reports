    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$div(
          ...
          actionButton(ns("redirect_build_reports"),"Build Reports",class ="home-btn"),
          actionButton(ns("redirect_learn_more"),"Learn More",class ="home-btn"),
...
mod_home_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$redirect_build_reports, {
      updateNavbarPage(session$rootScope(), "main_page", selected = "page_build_reports")
    })
    
    #redirect to Learn More Page - use rootScope to use parent level ids
    observeEvent(input$redirect_learn_more, {
      updateNavbarPage(session$rootScope(), "main_page", selected = "page_learn_more")
    })
