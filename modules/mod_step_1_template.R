mod_step_1_template_ui <- function(id, state) {
  ...
  tags$div(class = "form-content",
           h4(class = "form-step", "Step 1"),
           h2(class = "form-title", "Download Template"),
           p(
             class = "form-text",
             "Choose which language the report will be in. Download the Excel template and replace the example data in the 'Data' and 'Data Dictionary' sheets with your own.",
             br(), br(),
           ...
           div(
             ...
             downloadButton(ns("download_template"), "Download Template", style = "margin-top:20px;width:320px;")
           )
  )
}
...
mod_step_1_template_server <- function(id, state) {
  moduleServer(id, function(input, output, session) {
    ...
    state$language <- reactive({ input$language })
  })
}
